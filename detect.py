...
from utils.general import (LOGGER, check_img_size, check_requirements, cv2,
...
@torch.no_grad()
def run(
        weights=ROOT / 'helmet.pt',  # model.pt path(s)
    ...
    thread = threading.Thread(target=playsound, args=('alarm.wav',), daemon=True)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if c == 0 :
                        if not thread.is_alive():
                            thread = threading.Thread(target=playsound, args=('alarm.mp3',), daemon=True)
                            thread.start()
